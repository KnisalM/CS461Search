...
class AdjacencyGraphSearchGUI:
    def __init__(self, root):
        ...
        self.node_radius = 25           # bigger nodes
        self.font_size = 8              # smaller font
        ...
        # No graph initially
        self.graph_selection = None   # "existing" or "random"
        self.random_n = None
    ...
    def create_widgets(self):
        ...
        # Main display: canvas on left, metrics on right (no tree)
    ...
    # ------------------------------------------------------------------
    # Graph Loading and Drawing (with larger nodes, smaller font)
    # ------------------------------------------------------------------
    ...
    def load_existing_graph(self):
        ...
        # Manually load adjacencies, skipping missing nodes (like Hays)
        ...
        # Map lat/lon to canvas coordinates with larger margins
        self.node_positions = {}
        ...
        margin = 80   # extra space around edges
    ...
    def generate_random_graph(self):
        ...
        self.random_n = n
        self.graph = AdjacencyGraph()
        # Generate random positions with overlap avoidance
        margin = 80
        min_distance = 2 * self.node_radius + 10
        positions = {}
        attempts = 0
        for i in range(n):
            while True:
                ...
                for (px, py) in positions.values():
                    if math.hypot(x-px, y-py) < min_distance:
                        overlap = True
                        break
                if not overlap or attempts > 1000:
                    break
                attempts += 1
            name = str(i+1)
            # Store real coordinates (lat = y, lon = x) for heuristic & path cost
            self.graph.add_node(name, y, x)   # lat, lon
            positions[name] = (x, y)
    ...
    def draw_graph(self):
        """Draw nodes and edges on canvas with larger circles and smaller text."""
        ...
        for name, (x, y) in self.node_positions.items():
            ...
            # Draw text: "Start" / "Goal" for selected nodes, else node name
            if name == self.start_node:
            ...
            # Add X for dead ends (after search)
        ...
        if self.final_parent and self.goal_node in self.final_parent:
            ...
            for line_id, u, v in self.edge_lines:
                if u in path_nodes and v in path_nodes and (self.final_parent.get(v) == u or self.final_parent.get(u) == v):
    ...
    # ------------------------------------------------------------------
    # Start/Goal Selection (hover detection with larger radius)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Animation (same as before, but using larger node radius)
    # ------------------------------------------------------------------
    ...
    def display_metrics(self, metrics):
        ...
        # Add units to specific keys
            if key == 'runtime':
                formatted_value += " S"
                display_key = key
            elif key == 'Path Cost':
                display_key = "Path Cost (Euclidean Distance Traveled in total)"
            else:
                display_key = key
